...
Section~\ref{sec:background} gives the setting, Section~\ref{sec:conventions} separates the
conventions we adopt from the observations we contribute, and Section~\ref{sec:protocol} states
the protocol; the implications are drawn together in Section~\ref{sec:discussion}.
...
Figure~\ref{fig:protocol} summarises the design. We take a multi-beam power allocation problem
with amplifier distortion and run two arms under one protocol. The first maps the \emph{instance space}: for every configuration in a grid we ask
